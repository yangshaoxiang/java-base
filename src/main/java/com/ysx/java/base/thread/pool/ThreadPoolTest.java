...
public class ThreadPoolTest {
     ...
     *
     * 线程池何时回收空闲线程
     *  线程池回收空闲线程设计的巧妙，并非用了什么定时器之类，而是在线程到队列中获取任务时，先判断当前是否需要进行空闲回收(开启core空闲回收配置或者工作线程数大于core)，
     *  如果需要的话使用 poll 方法并设置阻塞时间为空闲回收时间，当超时未获取到任务就可以说线程是空闲状态，走上层方法 finally 回收线程
     *
     * 线程池使用AQS地方
     * 进入线程池的任务都会封装为一个Worker对象，该对象继承了AQS，默认state为-1，意义在于使这个刚创建的任务内部线程在未启动前不能被打断，在线程
     * 启动后可被打断
     * 实现方案很简单巧妙:新建时默认 state为-1 在打断时需要执行获取内部锁操作，这个获取逻辑很简单 使用cas更新 state 期望state为0，更新为1，因此在
     * 新建worker默认 state为-1 时是不可被打断的，在Worker内封装的线程启动时state置为0，可被打断
     *
     *
     */
